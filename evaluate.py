# """
# Evaluation script for w0wa power spectrum emulator.

# Validates emulator predictions against CAMB on the training set.
...
# testSet = utils.COLASet(target_z=utils.z_mps, cosmo_type=cosmo_type, nl_type=nl_type, start_batch=start_batch)

# iz = 0
# z = 0

# true_pks = testSet.pks_lin[:, iz, :]
# syren_pks = [pk_emu.get_pks(params, use_syren=True, model_type=cosmo_type)[2][iz] for params in testSet.lhs]
# pred_pks = [pk_emu.get_pks(params, use_syren=False, model_type=cosmo_type)[2][iz] for params in testSet.lhs]

# # print("Pred: ", pred_pks[0])
# # print("Syren: ", syren_pks[0])
# # print("True: ", true_pks[0])

# plt.figure(figsize=(8, 5))
# for i in range(200): 
#     plt.semilogx(testSet.ks, testSet.frac_pks[i, iz, :] - 1)
#     # plt.loglog(testSet.ks, testSet.mps_approxes[i, 0, :],linestyle="--")
# # plt.ylim(min(errors[0]), max(errors[0]))
# plt.xlabel("k")
# plt.ylabel(r"$P_\mathrm{CAMB}/P_\mathrm{Syren} - 1$")
# # plt.title(r"PCA Reconstruction Errors, $N_\mathrm{PC} = $" + f"{num_pcs}")
# plt.grid(True, alpha=0.3)
# plt.savefig(f"/gpfs/projects/MirandaGroup/victoria/cocoa/Cocoa/mps_emu/validation_figs/val_fracs_k500_{cosmo_type}_z{z}_expanded.pdf", bbox_inches="tight")
# plt.close()
...
# print("Emul: ", errors[0])
# print("Syren: ", errors_syren[0])

# # DEBUGGING FOR EMULATOR ERRORS
# print("\n=== DEBUGGING EMULATOR ERRORS ===")
# print("Shape of errors:", errors.shape)
# print("Min/Max of errors:", np.min(errors), np.max(errors))
# print("Shape of testSet.ks:", testSet.ks.shape)
# print("testSet.ks min/max:", testSet.ks[0], testSet.ks[-1])

# # Calculating error percentiles
...
# print("emup1 first 5 values:", emup2[:5])
# print("emup99 first 5 values:", emup98[:5])
# print("emup1 shape:", emup2.shape)
...
# line100 = mpl.lines.Line2D([], [], color=color100, label=r"$99\%$")
...
# axs.grid(alpha=0.4)
# # axs.set_xlim([testSet.ks[0], testSet.ks[-1]])  # Temporarily commented out
# axs.tick_params(axis='both', which='major', labelsize=17)

# plt.text(0.05, 0.95, fr'$z={z}$', transform=plt.gca().transAxes,
...
# print("Y-axis limits:", axs.get_ylim())
# print("X-axis limits:", axs.get_xlim())
# print("=== END DEBUGGING ===\n")

# plt.savefig(f"mps_emu/validation_figs/emul_errors_z{z:.3f}_{cosmo_type}_expanded.pdf", bbox_inches="tight")
# plt.close()
...
# axs.semilogx(testSet.ks, emup2_syren, color=color100)
# axs.fill_between(testSet.ks, emup10_syren, emup90_syren, color=color90)
# axs.fill_between(testSet.ks, emup25_syren, emup75_syren, color=color50, hatch="\\", edgecolor="lightgray")
# axs.semilogx(testSet.ks, emup98_syren, color=color100)
...
# axs.legend(handles=[hatch50,hatch90,line100], fontsize=axes_label_fontsize, loc="lower right")
# plt.savefig(f"mps_emu/validation_figs/syren_errors_z{z:.3f}_{cosmo_type}_expanded.pdf", bbox_inches="tight")
# plt.close()
